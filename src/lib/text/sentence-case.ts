...
const NORWEGIAN_ACRONYMS = new Set([
  ...
  "AI", "GDPR", "KS", "KA", "IT", "HR", "CEO", "CFO", "CTO",
  "PR", "TV", "PC", "USB", "API", "URL", "PDF", "PPTX",
  // Norwegian specific
  "NAV", "NRK", "DNB", "NSB", "SAS", "UiO", "NTNU", "UiB",
  "NOK", "SEK", "EUR", "USD", "GBP",
  // Units
  "KWH", "CO2", "M2", "M3",
]);
...
const PROPER_NOUNS = new Set([
  ...
  "Storbritannia", "Italia", "Spania", "Polen", "Nederland", "Belgia",
  "Europa", "Amerika", "Asia", "Afrika", "Australia", "Norden",
  // Major Norwegian cities
  "Oslo", "Bergen", "Trondheim", "Stavanger", "Kristiansand",
  "Tromsø", "Drammen", "Fredrikstad", "Sandnes", "Bodø",
  "Ålesund", "Tønsberg", "Haugesund", "Sandefjord", "Moss",
  "Sarpsborg", "Skien", "Arendal", "Gjøvik", "Hamar", "Lillehammer",
  "Molde", "Harstad", "Narvik", "Alta", "Hammerfest", "Kirkenes",
  // Other places
  "Melhus", "Frogner", "Majorstuen", "Grünerløkka", "Aker", "Bærum",
  // Companies/Organizations (common ones)
  "Microsoft", "Google", "Apple", "Amazon", "Meta", "Facebook", "LinkedIn",
  "Equinor", "Telenor", "Hydro", "Yara", "Storebrand", "Gjensidige",
  "Vipps", "Finn", "Schibsted", "Atea", "Visma", "Kahoot",
  // Government & public sector
  "Stortinget", "Regjeringen", "Kirkerådet", "Kirkemøtet",
  "Helse", "Helseforetaket", "Statsforvalteren", "Fylkeskommunen",
  // Organizations relevant to the current use case
  "Hovedorganisasjonen", "Gravplassforeningen",
...
const ALWAYS_LOWERCASE = new Set([
  // Prepositions
  "i", "på", "av", "til", "fra", "med", "for", "om", "ved", "etter",
  "under", "over", "mellom", "blant", "gjennom", "mot", "hos",
  // Conjunctions
  "og", "eller", "men", "så", "da", "når", "hvis", "fordi", "at",
  // Articles
  "en", "et", "ei", "den", "det", "de",
  // Verbs (common ones often incorrectly capitalized)
  "som", "er", "var", "har", "hadde", "blir", "ble", "kan", "skal",
  "vil", "må", "bør", "ikke",
  ...
  "viktige", "viktig", "nye", "ny", "neste", "første", "andre", "tredje",
  "store", "stor", "små", "liten", "god", "gode", "best", "beste",
  "fremtiden", "fremtid", "status", "oversikt", "oppsummering",
  "punkter", "trinn", "steg", "endringer", "vedtak", "beslutninger",
]);
...
export function toNorwegianSentenceCase(text: string): string {
  ...
  const result = words.map((segment) => {
    ...
    const isFirstWord = wordIndex === 0 || startsNewSentence(
...
export function isTitleCase(text: string): boolean {
  ...
  const capitalizedWords = words.filter(word => {
    ...
    return cleanWord.charAt(0) === cleanWord.charAt(0).toUpperCase() &&
